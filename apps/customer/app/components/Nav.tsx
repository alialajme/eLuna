...
function GuestNavButtons({ cartCount }: { cartCount: number }) {
  return (
    <>
...
async function ClerkNavButtons({ cartCount }: { cartCount: number }) {
    ...
    <header className="sticky top-0 z-30 border-b border-sand bg-ivory/95 backdrop-blur-sm">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 md:px-6">
        ...
        <div className="flex items-center gap-4">
          ...
          {hasClerkKeys ? (
            <ClerkNavButtons cartCount={cartCount} />
          ) : (
            <GuestNavButtons cartCount={cartCount} />
          )}
        </div>
      </nav>
    </header>
