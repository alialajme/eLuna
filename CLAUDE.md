...
| Payments Gateway | Real gateway integration (Stripe Card/Apple Pay/Google Pay, Tap Payments, Noqodi) | 🔲 Deferred dedicated phase — behind `PaymentGateway` iface (`apps/customer/app/lib/payment/gateway.ts`); needs PaymentMethod enum extension (first customer-side Prisma migration) + live credentials/webhooks (operator step). 8b's payment_methods lists these as "coming soon" until then. |
| 8c–8e | Logistics agent (8c, needs Phase 7), POS agent (8d), Agent handoff/orchestration + AISession persistence (8e) | 🔲 Not started |
