...
| 7 | Logistics (courier routing, tracking, returns) | 🟡 In progress — decomposed into 7a/7b |
...
| 7b | Returns & Refunds | 🔲 Not started — customer return request on delivered items → vendor/admin approve → Stripe refund (reuse `StripeGateway.refund`) + restock; refine `recomputeOrderStatus` for RETURNED. The `Return` model + `ReturnStatus` enum already exist (unused). |
