...
| Courier Integration | Real courier API machinery (abstraction + simulated fallback + scaffold + webhook) | ✅ Complete (2026-08-10, commits b49a0d8–885aa71) — credential-gated like payments. `Shipment` gains `externalRef?`/`labelUrl?` (`db push`). `apps/vendor/app/lib/courier/`: `CourierGateway` iface (`createShipment` → `created`/`manual`/`failed`; optional `parseWebhook` → `{match,status}\|{kind:"ignored"}`), `config` (`hasAramex()`…), **`SimulatedCourier`** (returns `manual` → vendor types tracking = 7a behavior, the no-keys default for every courier), **`AramexCourier`** scaffold/template (config-gated, `TODO(operator)` integration points), env-aware `factory` (`getCourierGateway`, never throws), shared **`applyShipmentStatus`** (idempotent; DELIVERED flips items + recomputes order — `markShipmentDelivered` refactored to reuse it). `createShipment(trackingNumber now optional)` calls the gateway: `created`→auto tracking/externalRef/labelUrl, `manual`→require vendor tracking. Webhook `api/webhooks/courier/[courier]` (400 bad-sig / 200 else, courier-scoped match, idempotent apply). FulfillmentPanel: optional tracking input + "Print label" link. `.env.example` + `docs/deployment/couriers.md`. Real per-courier API + keys + webhook registration = operator. **Note:** webhook used `!("match" in event)` (not `event.kind`) since the union isn't discriminated on a shared field. |
...
| Supplier S1 | Supplier persona — Foundation (4th separate-login app) | ✅ Complete (2026-08-10, commits 34fdd63–HEAD) — **new upstream B2B persona**: a materials/fabric supplier that serves Vendors (Amazon Seller-Central vs Vendor-Central split; Luna's Vendor = seller, Supplier = materials layer). First of 3 planned phases (S1 Foundation → S2 Materials Catalog → S3 Sourcing & Orders). Adds `SUPPLIER` to `UserRole` + new `SupplierStatus` enum + `Supplier` model (mirrors `Vendor`: `companyName`/`companySlug`, adds `materialTypes String[]`, **no `commissionRate`** — wholesale, deferred to S3; `db push`). `@e-luna/auth` `UserRole`/`ROLES`/`ClerkSessionClaims.supplierId` + `getAuthUser()` now forwards `supplierId`. **New app `apps/supplier` (supply.luna.ae, dev port 3003)** — a 4th separately-authenticated app (persona logins kept separate, per user's Amazon-style security ask; separation is by each app's own Clerk instance, NOT a middleware role-claim gate). Clones vendor: hand-rolled `clerkMiddleware` (signed-in + public routes), sign-in, 2-step onboarding wizard (company details + material-type multiselect → `createSupplier`; then MFA guidance → `/pending`), status-gated dashboard shell with **"coming soon" seams** for S2 (materials) + S3 (orders). `createSupplier` mirrors `createVendor` (`safeCurrentUser`-scoped userId, slug regex, `sanitizeMaterialTypes` allowlist, duplicate-account + slug pre-checks, P2002). Admin: `actions/suppliers.ts` (approve/reject/suspend/reactivate, **ADMIN-gated via `getAuthUser`**), `/suppliers/approvals` page + `SupplierActions` (clone of sellers/`VendorActions`) + "🧵 Suppliers" nav. `pending` page reads `?reason=` (under-review / suspended / rejected copy). `.env.example` note: supplier app uses its OWN Clerk instance in prod. Verified: full-workspace `tsc --noEmit` 0 + `pnpm lint` 4/4 + gitleaks clean. **Deferred:** materials catalog (S2), vendor Sourcing + `MaterialOrder` (S3), supplier payouts/analytics/AI agent. Minor follow-ups noted in final review: approvals card could show `createdAt` (M1); sidebar active-state has no explicit `/suppliers/*` case (M2, fine for single route). |
| Supplier S2 | Supplier — Materials Catalog | ✅ Complete (2026-08-10, commits cbe8842–HEAD) — supplier can build/manage a catalog of **flat, single-SKU material listings** (per the brainstorming decision: no `Product→ProductVariant`-style variants). New `Material` model (`db push`): `supplierId` (cascade), `name`, `slug @unique`, `materialType String` (∈ shared `MATERIAL_TYPES`), `color?`, `composition?`, `unit MaterialUnit`, `wholesalePrice Decimal(10,2)`, `moq Int @default(1)` (captured now, **enforced in S3**), `stock`, `description?`, `images Json` (URL strings, like `Product.aiImages`), `status MaterialStatus @default(DRAFT)`, indexes `[supplierId]`/`[status]`/`[supplierId,status]`. New enums `MaterialStatus {DRAFT ACTIVE ARCHIVED}` (no admin moderation — suppliers vetted at account approval) + `MaterialUnit {METER YARD ROLL PIECE SPOOL}`; `Supplier.materials` back-relation. `apps/supplier/app/lib/materials.ts` += `isMaterialType`, `MATERIAL_UNITS`, `MaterialUnitValue`. `actions/material.ts`: `createMaterial`/`updateMaterial`/`archiveMaterial`/`deleteMaterial` — **security invariant: `supplierId` server-resolved (never a client param); every mutation ownership-checks `material.supplierId === supplier.id` AND requires `supplier.status === "ACTIVE"`** via `resolveActiveSupplier()`; `validate()` (name 2–80, price>0 finite, moq≥1 int, stock≥0 int, materialType allowlist, unit ∈ `MATERIAL_UNITS` set, color≤60/composition≤120/description≤600, images trimmed+≤8), `generateSlug` collision-suffix. Shared `MaterialForm` island (create/edit; type/unit selects, image URLs one-per-line textarea, Draft/Active toggle; Archive [hidden when already archived] + Delete in edit). RSC pages: `/materials` list (status-tab filter, empty state, supplier-scoped) + `new` + `[id]` (ownership → `notFound()`; `Decimal`→`Number`, `images` Json→`string[]`, ARCHIVED→DRAFT remap = un-archive-by-editing). Sidebar "🧵 Materials" promoted from SOON to real link; dashboard Materials card now live (count → `/materials`). Verified full-workspace `tsc` 0 + `pnpm lint` 4/4 + gitleaks clean. Final review confirmed multi-tenancy solid; fixed inline: I1 (unit validation now derives from `MATERIAL_UNITS`, no drift), M2 (server length guards), M3 (price `min="0.01"`), M1 (hide Archive on archived). Accepted as-is: I2 (edit-page `generateMetadata` shows name pre-ownership-check — title-bar-only, identical to the vendor product sibling convention). **Deferred to S3:** vendor Sourcing browse, `MaterialOrder`, MOQ enforcement, supplier payouts. Spec+plan dated 2026-08-10. |
| Supplier S3 | Supplier — Sourcing & Orders (final supplier phase) | ✅ Complete (2026-08-11, commits c1eb4b9–HEAD) — the **B2B ordering loop**: vendors buy materials from suppliers. New models (`db push`): `MaterialOrderStatus {PENDING ACCEPTED SHIPPED COMPLETED CANCELLED REJECTED}`, `MaterialOrder` (header: `vendorId` buyer + `supplierId` seller + `status` + `total Decimal` + `note?` [vendor→supplier] + `trackingNote?` [supplier→vendor]; indexes `[vendorId]`/`[supplierId]`/`[supplierId,status]`/`[vendorId,status]`), `MaterialOrderItem` (**snapshotted** line: `materialId String?` `onDelete:SetNull` + `materialName`/`unit`/`unitPrice` snapshots + `quantity`; order history survives material edit/delete). Back-relations on Vendor/Supplier/Material. **Single-material quick order** (no cart — brainstorming decision; schema is header+lines so multi-line is possible later). **No payment** (PO-record model; settlement offline) + **no courier gateway** (free-text `trackingNote`) — both deliberately deferred. **Vendor (`apps/vendor`):** `actions/sourcing.ts` `createMaterialOrder(materialId,qty,note?)` (vendorId server-resolved; material ACTIVE + supplier ACTIVE; qty int ∈ [moq,stock]; snapshots; Decimal money via `.mul`; PENDING; **no stock change at placement**) + `cancelMaterialOrder` (owner + PENDING→CANCELLED); `(dashboard)/sourcing/{page[browse ACTIVE+ACTIVE, type filter],[id][detail+PlaceOrderForm, notFound unless ACTIVE],orders/page,orders/[id][ownership→notFound, Cancel while PENDING]}`; `PlaceOrderForm` + `CancelOrderButton` islands; Sidebar "🧶 Sourcing". **Supplier (`apps/supplier`):** `actions/incoming-order.ts` `acceptMaterialOrder`/`rejectMaterialOrder`/`shipMaterialOrder(trackingNote?)`/`completeMaterialOrder` (supplierId server-resolved; ownership + precursor-status guards PENDING→ACCEPTED/REJECTED→SHIPPED→COMPLETED); **accept commits stock race-safely inside `$transaction`**: guarded `material.updateMany({where:{id,stock:{gte:qty}},data:{decrement}})` + **atomic `materialOrder.updateMany({where:{id,status:"PENDING"}})` status guard with compensating stock `increment` on lost race** (fixes the C1 double-accept TOCTOU the reviewer caught); `(dashboard)/orders/{page[status filter],[id][ownership→notFound]}` + `OrderActions` island (status-appropriate buttons); Sidebar "📋 Incoming Orders" promoted (SOON_ITEMS removed — no more coming-soon seams); dashboard card live (pending count). Verified full-workspace `tsc` 0 + `pnpm lint` 4/4 + gitleaks clean. Final review: multi-tenancy solid, lifecycle correct; fixed inline C1 (atomic accept status guard), I1 (typed `MaterialOrderStatus` filter cast), I2 (Decimal `.mul` money, no float). Accepted as noted: M1 (supplier tabs omit CANCELLED/REJECTED — intentional), M2 (`return null` in list pages — matches existing pattern), M3 (`label()` dup'd 4×), M4 (`unit:string` display prop). **Supplier persona (S1+S2+S3) now COMPLETE.** Further deferred: supplier payouts/analytics for material sales, Supplier AI agent, returns on material orders, multi-line carts, MOQ price-breaks. Spec `docs/superpowers/specs/2026-08-10-supplier-s3-sourcing-orders-design.md` + plan `.../plans/2026-08-10-supplier-s3-sourcing-orders.md`. |
| Supplier AI Agent | Advisory assistant in the supplier dashboard | ✅ Complete (2026-08-11, commits 7841091–HEAD) — **5th live AI agent**, clones the Seller (8a) pattern. `packages/ai/src/agents/supplier.ts`: `buildSupplierTools(supplierId)` + `runSupplierAgent(messages,{supplierId,onFinish})` (streamText, `maxSteps:5`, `SUPPLIER_SYSTEM` — advisory tone). **ADVISORY / READ-ONLY** (like 8b/8c — never mutates; accept/ship stay deterministic dashboard actions). 4 tools, **`supplierId` closure-captured, NEVER an LLM param**, every query scoped to it: `flag_low_material_stock(threshold=5)` (ACTIVE materials, stock<threshold), `pending_orders()` (PENDING MaterialOrders + buyer store + total + ageDays), `material_sales(days=30)` (units+revenue from ACCEPTED/SHIPPED/COMPLETED + top-5 materials by revenue), `benchmark_material_price(materialId)` (**ownership-checked** `where:{id,supplierId}` first, then median wholesalePrice of same-materialType ACTIVE materials across suppliers). 8e persistence: `"SUPPLIER"` added to BOTH `AGENT_TYPES` (session.ts) **and the Prisma `AIAgentType` enum** (the `AISession.agentType` column type — tsc caught this; `db push`). Supplier app now depends on `@e-luna/ai` (+ `ai` SDK + transpile) → **requires `ANTHROPIC_API_KEY` at runtime** like vendor/customer (config.ts throws at import if unset; added `ANTHROPIC_API_KEY` to `.env.example` — was a pre-existing gap). `apps/supplier/app/api/assistant/route.ts` (POST; resolves supplier server-side, **401 no user / 403 not-a-supplier / 403 not-ACTIVE** [status re-checked since API routes bypass the layout gate — review I1] / 500; `persistOnFinish(user.id,"SUPPLIER",messages)`) + `api/ai-history/route.ts` (GET, `isAgentType`-guarded). `LunaChatWidget` (`agentType="SUPPLIER"`, "Supplier Assistant") mounted in the dashboard layout. Verified `tsc` 0 + `pnpm lint` 4/4 + gitleaks clean. Final review: security invariant airtight, read-only confirmed, 8e wired both layers; fixed I1 (ACTIVE gate on route). Left as noted (all mirror the seller/vendor siblings, would diverge if changed alone): M1 (even-length median upper-bias), M2 (benchmark peers include own material), M3 (`material_sales` filters on `createdAt` not `updatedAt`), M4 (ai-history returns 200+[] for bad agentType). **Deferred:** supplier payouts/analytics, returns on material orders. Spec `docs/superpowers/specs/2026-08-11-supplier-ai-agent-design.md` + plan `.../plans/2026-08-11-supplier-ai-agent.md`. |
| Supplier E-Invoicing (UAE FTA) | 1st "Supplier OS ↔ UAE systems" integration | ✅ Complete (2026-08-11, commits 98d83d7–HEAD) — supplier issues a **UAE tax-compliant invoice** (5% VAT, TRN) for a fulfilled S3 `MaterialOrder`, via the project's **credential-gated gateway pattern** (as Payments/Couriers). **Honesty boundary:** `SimulatedEInvoice` works FULLY offline (numbering, VAT, persisted, printable document = the compliant invoice); **FTA/Peppol network transmission is the config-gated `FtaEInvoice` scaffold** (`TODO(operator)`, `hasFtaAccessPoint()` → only used when `FTA_ACCESS_POINT_URL`+`FTA_API_KEY` set). Schema (`db push`): `Supplier.trn String?`, `enum InvoiceStatus {DRAFT ISSUED VOID}`, `MaterialInvoice` (`invoiceNumber @unique`, `materialOrderId @unique` [one/order], party+money snapshots, `subtotal`/`vatRate @default 0.05`/`vatAmount`/`total` Decimal(10,2), `lines Json`, `externalRef?`, `issuedAt`), back-relations `MaterialOrder.invoice?` + `Supplier.invoices[]`. `apps/supplier/app/lib/einvoice/` = `gateway.ts` (iface `issue(params)→{issued,externalRef}|{failed,error}`) + `simulated.ts` (no-key default, never fails) + `fta.ts` (scaffold) + `config.ts` + `factory.ts` (`getEInvoiceGateway()` never throws). `actions/invoice.ts`: `setSupplierTrn` (15-digit `/^\d{15}$/`) + `issueMaterialInvoice(orderId)` — **supplierId server-resolved**; guards owner + order∈{ACCEPTED,SHIPPED,COMPLETED} + TRN present + one-per-order; **VAT = NET** (`subtotal=order.total`, `vat=round(subtotal*.05,2)`, `total=subtotal+vat`); sequential number `<SLUG4>-<YYYY>-<NNNN>` (count supplier's invoices this year +1) with **P2002 retry that distinguishes materialOrderId [→"already invoiced"] vs invoiceNumber [→recompute]** (review I1 fix). UI: `(dashboard)/settings` (TrnForm + FTA status Simulated/Connected), `invoices/{page,[id]}` ([id] = **printable FTA-compliant Tax Invoice**: TRN/buyer/number/date/lines/Net/VAT(5%)/Total, `PrintButton`→window.print, ownership→notFound), order `[id]` gains **Issue/View tax invoice** control (TRN-gated `IssueInvoiceButton`), Sidebar += 🧾 Invoices + ⚙️ Settings. `.env.example` FTA vars + `docs/deployment/einvoicing.md` operator guide. Verified full `tsc` 0 + `pnpm lint` 4/4 + gitleaks clean. Final review (sonnet): NO critical; multi-tenancy airtight, VAT correct, numbering constraints DB-enforced, honesty boundary intact. Fixed inline: I1 (P2002 target discrimination). Noted for FtaEInvoice activation: I2 (gateway called before DB write, not in a tx — persist-DRAFT→transmit→ISSUED before going live; documented in einvoicing.md). Left as convention: I3 (`.catch(()=>null)` masks DB errors — matches courier/payment siblings), M1–M3 (extra trn round-trip; `return null` in list pages). **Next UAE integrations (deferred):** ERP/inventory sync, supplier trade-licence verification, customs/Dubai Trade. Spec `docs/superpowers/specs/2026-08-11-supplier-einvoicing-design.md` + plan `.../plans/2026-08-11-supplier-einvoicing.md`. |
| Vendor Invoicing (customer orders) | Vendor issues UAE tax invoice for their portion of a customer Order; customer downloads it | ✅ Complete (2026-08-11, commits cfd068e–3090f5a, pushed) — sibling of Supplier E-Invoicing, but **per-vendor-per-order** + **VAT-INCLUSIVE (retail)**. A customer `Order` can span multiple vendors → **each vendor issues one invoice for their line items** (a 2-vendor order = 2 independent invoices). **Extracted the gateway to shared `packages/einvoice` (`@e-luna/einvoice`)** and repointed the supplier to it — the gateway `IssueParams` generalized to `seller:{name,trn}`/`buyer:{name}`/`lines:{description,quantity,unitPrice,lineTotal}[]` (supplier maps supplier→seller/vendor→buyer, its `MaterialInvoice.lines` JSON keeps the richer name/unit unchanged). New shared **`TaxInvoiceDocument`** in `@e-luna/ui` (pure printable render, no `"use client"`, used by BOTH vendor + customer invoice pages). Schema (`db push`): **`Vendor.trn String?`** + new **`OrderInvoice`** (`invoiceNumber @unique`, `@@unique([orderId,vendorId])` [compound key `orderId_vendorId`], `vendorName`/`vendorTRN`/`customerName` snapshots, `subtotal`[=net]/`vatRate @default 0.05`/`vatAmount`/`total` Decimal(10,2), `lines Json`, `externalRef?`, `issuedAt`), back-relations `Order.invoices[]` + `Vendor.orderInvoices[]` (no `InvoiceStatus` reuse — always issued). Vendor `apps/vendor/app/actions/invoice.ts`: `setVendorTrn` (`/^\d{15}$/`) + `issueOrderInvoice(orderId)` — **vendorId server-resolved (ACTIVE)**; loads order items **where vendorId===vendor.id** + `address.fullName`; guards vendor-has-items + status∈{CONFIRMED,PROCESSING,SHIPPED,DELIVERED} + TRN present + no existing invoice for (order,vendor); **INCLUSIVE VAT**: `gross=Σ lineTotal`, `net=round(gross/1.05,2)`, `vat=round(gross−net,2)`, `total=gross`, `subtotal` stored=net, **gateway `subtotal` param passed = net**; sequential `<STORE4>-<YYYY>-<NNNN>` with **P2002 retry distinguishing (orderId,vendorId)[→"already invoiced", terminal] vs invoiceNumber[→recompute once]**. Vendor UI: `(dashboard)/invoices/{page,[id]}` ([id] renders `TaxInvoiceDocument`, ownership→notFound), `IssueInvoiceButton` island (TRN-gated, redirects to invoice on success), order `[id]` sidebar gets Issue/View control, Settings gets `TrnForm` + FTA status line, Sidebar += 🧾 Invoices. Customer UI: `orders/[id]` lists "Download tax invoice — <vendor>" per `OrderInvoice`; new `orders/[id]/invoice/[invoiceId]/page.tsx` printable page **double-checks `inv.orderId===id` AND `inv.order.customerId===profile.id`**→notFound (read-only, customers never issue). Reuses `FTA_ACCESS_POINT_URL`/`FTA_API_KEY` (no new env). Verified full-workspace `tsc` 0 + `pnpm lint` 4/4 (only pre-existing `<img>` warnings) + gitleaks clean; supplier e-invoicing still compiles after extraction. Spec `docs/superpowers/specs/2026-08-11-vendor-invoicing-design.md` + plan `.../plans/2026-08-11-vendor-invoicing.md`. **Deferred (YAGNI):** auto-issue on payment, shipping/discount allocation across vendor invoices, credit notes/multi-currency/non-5%, real FTA transmission (`FtaEInvoice` scaffold unchanged). **This was feature #1 of 4** the user queued; next: #2 Supplier courier (supplier→vendor Aramex/DHL), #3 Supplier dropship→customer, #4 surface NeoPay in checkout picker. (I1 fix — compound P2002 constraint-name match — landed in the Supplier Courier branch, commit 959815f.) |
| Supplier Courier (supplier→vendor) | Supplier ships a fulfilled MaterialOrder to the buying vendor via a real courier (Aramex/DHL); vendor sees live tracking | ✅ Complete (2026-08-12, commits 79dcb0e–959815f) — **feature #2 of 4.** Extends the vendor Courier Integration to the **supplier→vendor** leg. **Extracted the pure gateway to shared `packages/courier` (`@e-luna/courier`)**: `gateway.ts` (iface + `CreateShipmentParams{reference,courier,destination,weightKg?}` — `reference` generalizes the old `orderId`; **neutral `CourierDeliveryStatus = "in_transit"|"delivered"|"exception"`** replacing the customer-order-specific `ShipmentStatus` so the package NO LONGER imports `@e-luna/db`), `config.ts` (`hasAramex`/`hasFetchr`/`hasQuiqup`/`hasEmiratesPost`/`hasDhl`), `simulated.ts` (manual fallback), `aramex.ts` + **new `dhl.ts`** scaffolds (config-gated, `TODO(operator)`), `factory.ts` (`getCourierGateway` wires aramex+dhl, never throws → Simulated). **Repointed the vendor**: deleted `apps/vendor/app/lib/courier/{gateway,config,simulated,aramex,factory}.ts` (kept app-local `apply-status.ts`); `actions/shipment.ts` + webhook import from `@e-luna/courier`; the vendor webhook now **maps neutral→ShipmentStatus** (`delivered`→DELIVERED else IN_TRANSIT) before `applyShipmentStatus`; `createShipment` call uses `reference:`. Schema (`db push`): `MaterialOrder` += `courier`/`trackingNumber`/`externalRef`/`labelUrl` (optional; `trackingNote` kept for free-text). **Supplier**: `shipMaterialOrder(orderId, {courier, trackingNumber?, trackingNote?})` — **supplierId server-resolved**, guards owned + ACCEPTED, validates `getCourier`, calls gateway (`failed`→error, `created`→auto tracking/externalRef/labelUrl, `manual`→require trackingNumber), sets SHIPPED + fields; supplier-local `lib/courier/apply-status.ts` `applyMaterialOrderDelivery` (idempotent guarded `updateMany` SHIPPED→COMPLETED); webhook `api/webhooks/courier/[courier]` (400 bad-sig, courier-scoped `findFirst` match, only `delivered` advances state); `OrderActions` island ACCEPTED branch now = courier `<select>` (from `@e-luna/ui/couriers` COURIERS) + tracking# + note; order-detail shows courier name + `trackingUrl` deep-link + Print-label + note. **Vendor buyer view** (`sourcing/orders/[id]`) shows courier + tracking deep-link + status (falls back to bare `trackingNote`). **Honesty boundary:** no courier keys → Simulated (supplier types tracking = old behavior); real Aramex/DHL API + webhook = operator (`docs/deployment/couriers.md`, `.env.example` DHL vars). **Vendor has NO structured address model** → real-courier destination is best-effort (`storeName` only) — documented operator follow-up. Verified full-workspace `tsc` 0 + `pnpm lint` 4/4 + no stale source imports + gitleaks clean. Spec `docs/superpowers/specs/2026-08-11-supplier-courier-design.md` + plan `.../plans/2026-08-11-supplier-courier.md`. **Deferred:** real API calls, vendor "confirm received" action, returns/RMA on material orders, multi-parcel, vendor address model. (Post-review fix-forward commit 632f170 addressed the courier review's I1 [robust P2002 target match — normalize array-or-string], I2 [`shipMaterialOrder` atomic `updateMany({status:ACCEPTED})` guard], M3 [`completeMaterialOrder` atomic guard], M1/M2 docs.) |
| Supplier Dropship → Customer | Vendor lists a product but a supplier ships it directly to the customer (classic dropshipping); supplier invisible to customer | ✅ Complete (2026-08-12, commits b2b3c9e–057bd12) — **feature #3 of 4.** "Vendor lists, supplier fulfils." **Money deferred** (checkout untouched; customer pays vendor retail; wholesale settlement offline — PO-record philosophy). **Extracted the customer-order state machine to `@e-luna/db`**: `packages/db/src/order-status.ts` = `recomputeOrderStatus` + `applyShipmentStatus` (moved from `apps/vendor/app/lib/order-status.ts` + `apps/vendor/app/lib/courier/apply-status.ts`; internal import is `./client`); exported from the `@e-luna/db` barrel; repointed vendor `actions/shipment.ts`, `actions/returns.ts` (also an importer — plan-gap caught during build), and the courier webhook. Schema (`db push`): `Product.dropshipSupplierId String?` (+ relation `SupplierDropshipProducts`, index) + `Shipment.supplierId String?` (+ relation `SupplierCustomerShipments`, index; `Shipment.vendorId` stays required — a dropship shipment sets BOTH: vendorId=listing vendor [keeps vendor queries + recompute working], supplierId=fulfiller); `Supplier.dropshipProducts`/`customerShipments` back-relations. **Vendor**: product form gains a "Fulfilment" `<select>` (In-house | Dropship — <supplier>) fed by ACTIVE suppliers from new/[id] pages; `product.ts` `resolveDropshipSupplierId` validates null-or-ACTIVE + persists; `createShipment` EXCLUDES dropship items (`variant.product.dropshipSupplierId: null`) so the vendor can't ship them; order-detail `FulfillmentPanel` shows a read-only "N item(s) fulfilled by <supplier> (dropship)" note + drops them from the shippable set. **Supplier**: `actions/dropship.ts` `shipDropshipItems({orderId,vendorId,courier,trackingNumber?,trackingNote?})` — **supplierId server-resolved**; loads the order's items scoped to `vendorId` + `variant.product.dropshipSupplierId==supplier.id` + unshipped + order∈{CONFIRMED,PROCESSING}; `getCourier` valid; gateway (`@e-luna/courier`, `failed`/`created`/`manual`); `$transaction` creates `Shipment{vendorId,supplierId,IN_TRANSIT}` + flips items `shipmentId`+SHIPPED; then `recomputeOrderStatus`. `markDropshipDelivered(shipmentId)` ownership-checks `shipment.supplierId` → `applyShipmentStatus(...,"DELIVERED")`. New `(dashboard)/fulfilment` "Customer Orders" queue (TO SHIP groups by (order,vendor) with customer address + a ShipGroup island; SHIPPED list + DeliverButton) + Sidebar "📦 Customer Orders". **One shipment per (order,vendor) group** (1 tracking#:1 Shipment). **Customer: NO change — supplier invisible** (order page renders shipments generically; `supplierId` never surfaced). **Delivery manual this phase** (supplier button); real-courier webhook auto-delivery for dropship deferred. Verified full-workspace `tsc` 0 + `pnpm lint` 4/4 + no stale source imports + gitleaks clean. Spec `docs/superpowers/specs/2026-08-12-supplier-dropship-design.md` + plan `.../plans/2026-08-12-supplier-dropship.md`. **Deferred:** supplier payout/wholesale ledger, per-order accept/reject, dropship returns/RMA, courier-webhook auto-delivery. **Only #4 left: surface NeoPay in checkout picker (Apple/Google Pay already work with a real Stripe key).** |
